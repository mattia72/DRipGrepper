...
type
	TFileAttributes = (dfaReadonly, dfaHidden, // Hidden files
...
type
	...
	TSimpleDirEnumerator = class
		...
		public
			...
			class function Execute(const _Mask : string; _List : TStrings; _MayHaveAttr : TFileAttributeSet = ALL_FILES_ATTRIB_SET;
				_IncludePath : Boolean = False; _Sort : Boolean = True; _Recursive : Boolean = False) : Integer;
			/// <summary>
			/// creates a TSimpleDirEnumerator, calls its FindAll method and frees it
			/// @param List is a string list to which the files will be appended, may be nil
			/// @param IncludePath determines whether the List of filenames includes the full path or not </summary>
			class function EnumFilesOnly(const _Mask : string; _List : TStrings; _IncludePath : Boolean = False; _Sort : Boolean = True;
				_Recursive : Boolean = False) : Integer; overload;
			class function EnumFilesOnly(const _Mask : string; _IncludePath : Boolean = False; _Sort : Boolean = True;
				_Recursive : Boolean = False) : TStringArray; overload;
			/// <summary>
			/// Calls the given callback for all files matching the mask. To Abort, raise EAbort. </summary>
			class procedure EnumFilesOnly(const _Mask : string; _Callback : TOnFileEnumCallback; _IncludePath : Boolean = False;
				_Sort : Boolean = True); overload;
			/// <summary>
			/// creates a TSimpleDirEnumerator, calls its FindAll method and frees it
			/// @param List is a string list to which the files will be appended, may be nil
			/// @param IncludePath determines whether the List of filenames includes the full path or not </summary>
			class function EnumDirsOnly(const _Mask : string; _List : TStrings; _IncludePath : Boolean = False; _Sort : Boolean = True)
				: Integer; overload;
			class function EnumDirsOnly(const _Mask : string; _IncludePath : Boolean = False; _Sort : Boolean = True)
...
type
	...
	TCopyProgressResult = (prContinue, // continue with the copy/move operation
	...
	TCopyProgressReason = (prChunkFinished, // a chunk of the file has been copied
...
type
	...
	TCopyFileProgressEvt = procedure(_Status : TCopyProgressStatus; var _Continue : TCopyProgressResult) of object;
	...
	IUniqueTempDir = interface
		['{D9A4A428-66AE-4BBC-B1CA-22CE4DE2FACB}']
...
type
	...
	TMoveFileExFlags = (mfCopyAllowed, { mfCreateHardlink, } mfDelayUntilReboot, mfFailIfNotTrackable, mfReplaceExisting, mfWriteThrough);
	...
	TMoveFileWithProgressFlags = (mfwFailIfExists,
		/// < fail if the destination file already exists
		mfwAllowCopy,
		/// < allow using copy and delete if necessary
		mfwDelayUntilReboot,
		/// < wait until next reboot for moving the file
		mfwWriteThrough,
...
type
	...
	TFileSystem = class
		public
			...
			class function MoveFile(const _Source, _Dest : string; _RaiseException : Boolean) : Boolean; overload; deprecated;
			/// <summary>
			/// Moves the file Source to Dest using the Windows MoveFile function.
			/// @param Source is a string containing the name of the existing file
			/// @param Dest is a string containing the destination file name
			...
			/// but just return false if moving the file fails.
			/// @returns true, if the file could be moved, false otherwise. </summary>
			class function MoveFile(const _Source, _Dest : string; _ErrorHandling : TErrorHandlingEnum = ehRaiseException)
				: Boolean; overload;
			...
			/// but just return false if moving the file fails.
			/// @returns true, if the file could be moved, false otherwise. </summary>
			class function MoveFileEx(const _Source, _Dest : string; _Flags : TMoveFileExFlagSet; _RaiseException : Boolean) : Boolean;
				overload; deprecated;
			/// <summary>
			/// Moves the file Source to Dest using the Windows MoveFileEx function.
			/// @param Source is a string containing the name of the existing file
			...
			class function CopyFile(const _Source, _Dest : string; _FailIfExists : Boolean; _RaiseException : Boolean = True;
				_ForceOverwrite : Boolean = False) : Boolean; overload; deprecated;
			/// <summary>
			/// Copies the file Source to Dest using the Windows CopyFile function.
			/// @param Source is a string containing the name of the existing file
			/// @param Dest is a string containing the destination file name
			...
			/// @returns true, if the file could be copied, false otherwise.
			/// @raises  EOSError if an error occurs and cfwRaiseException was passed </summary>
			class function CopyFile(const _Source, _Dest : string; _IfExists : TCopyFileFlagIfExists = cfeFailIfExists;
			...
			class function CopyFileWithProgress(const _Source, _Dest : string; _Progress : TCopyFileProgressEvt;
				_IfExists : TCopyFileFlagIfExists = cfeFailIfExists; _ErrorHandling : TErrorHandlingEnum = ehRaiseException;
			...
			/// @returns true, if the directory was created
			/// @raises EOSError if there was an error and RaiseException was true </summary>
			class function CreateDir(const _DirectoryName : string; _ErrorHandling : TErrorHandlingEnum = ehRaiseException)
				: Boolean; overload;
			...
			class function ForceDir(const _DirectoryPath : string; _ErrorHandling : TErrorHandlingEnum = ehRaiseException) : Boolean;
			/// <summary>
			...
			/// @returns true, if the directory was created
			/// @raises EOSError if there was an error and ErrorHandling was ehRaiseException </summary>
			class function ForceDirFor(const _FullFilename : string; _ErrorHandling : TErrorHandlingEnum = ehRaiseException) : Boolean;

			/// <summary> See also CopyMatchingFiles </summary>
			class function CopyDir_Old(const _SrcDir, _DestDir : string; _CreateIntermediate : Boolean; _RaiseException : Boolean)
				: Boolean; deprecated;
			...
			/// @raises EOSError if there was an error and RaiseException was true </summary>
			class function SetReadonly(const _Filename : string; _Value : Boolean; _RaiseException : Boolean) : Boolean; overload;
				deprecated;
			/// <summary>
			/// Sets a file's readonly flag
			/// @param Filename is the file to change
			...
			/// @returns true, if the readonly flag has been changed
			/// @raises EOSError if there was an error and RaiseException was true </summary>
			class function SetReadonly(const _Filename : string; _Value : Boolean; _ErrorHandling : TErrorHandlingEnum = ehRaiseException)
				: Boolean; overload;
			...
			/// @raises EOSError if there was an error and RaiseException was true </summary>
			class function DeleteMatchingFiles(const _Dir, _Mask : string; _RaiseException : Boolean = True; _Force : Boolean = False)
				: Integer; overload;
			...
			/// the full paths.
			/// @returns the number of matching files </summary>
			class function FindMatchingFiles(const _Mask : string; _sl : TStrings; _IncludePath : Boolean = False; _Sort : Boolean = True)
				: Integer; overload;
			class function FindMatchingFiles(const _Mask : string; _IncludePath : Boolean = False; _Sort : Boolean = True)
				: TStringArray; overload;
			...
			/// @NOTE: The Filename does not contain the path!
			...
			class function FileExists(const _Filename : string; _RaiseException : Boolean) : Boolean; overload; deprecated;
			// use AssertFileExists instead
			/// <summary>
			...
			class function DirExists(const _DirName : string; _RaiseException : Boolean) : Boolean; overload; deprecated;
			// use AssertDirExists instead
			/// <summary>
			...
			/// @raises EOSError if there was an error and RaiseException was true </summary>
			class function RemoveDir(const _DirName : string; _RaiseException : Boolean = True; _Force : Boolean = False) : Boolean;
			...
			/// @returns True, if the directory could be deleted, false otherwise.
			/// @raises EOSError if there was an error and RaiseException was true </summary>
			class function DelDirTree(const _DirName : string; _RaiseException : Boolean = True; _Force : Boolean = False) : Boolean;
			...
			class function IsValidFilename(const _s : string; _AllowDot : Boolean = True; _AllowPathChars : Boolean = False)
				: Boolean; overload;
			/// <summary>
			/// checks whether the given string is a valid filename (without path), that is
...
function TextFile_IsOpen(var _File : TextFile) : Boolean;
...
constructor TSimpleDirEnumerator.Create(const _Mask : string; _MayHaveAttr : TFileAttributeSet = ALL_FILES_ATTRIB_SET);
begin
	FMask := _Mask;
	FMustHaveAttr := [];
...
class function TSimpleDirEnumerator.EnumDirsOnly(const _Mask : string; _List : TStrings; _IncludePath, _Sort : Boolean) : Integer;
var
...
class procedure TSimpleDirEnumerator.EnumFilesOnly(const _Mask : string; _Callback : TOnFileEnumCallback; _IncludePath : Boolean = False;
	_Sort : Boolean = True);
var
	enum : TSimpleDirEnumerator;
	List : TStringList;
	i : Integer;
begin
	enum := TSimpleDirEnumerator.Create(_Mask, [dfaArchive]); // for files
	try
		List := TStringList.Create;
		try
...
class function TSimpleDirEnumerator.EnumFilesOnly(const _Mask : string; _IncludePath : Boolean; _Sort : Boolean; _Recursive : Boolean)
	: TStringArray;
var
	sl : TStringList;
begin
...
class function TSimpleDirEnumerator.EnumFilesOnly(const _Mask : string; _List : TStrings; _IncludePath, _Sort : Boolean;
	_Recursive : Boolean) : Integer;
begin
	Result := Execute(_Mask, _List, [dfaArchive], _IncludePath, _Sort, _Recursive);
end;

class function TSimpleDirEnumerator.Execute(const _Mask : string; _List : TStrings; _MayHaveAttr : TFileAttributeSet = ALL_FILES_ATTRIB_SET;
	_IncludePath : Boolean = False; _Sort : Boolean = True; _Recursive : Boolean = False) : Integer;
var
	enum : TSimpleDirEnumerator;
	List : TStringList;
...
function GetVolumeInformation(lpRootPathName : PChar; lpVolumeNameBuffer : PChar; nVolumeNameSize : DWORD; lpVolumeSerialNumber : PDWORD;
	lpMaximumComponentLength, lpFileSystemFlags : LPDWORD; lpFileSystemNameBuffer : PChar; nFileSystemNameSize : DWORD) : BOOL; stdcall;
	external kernel32 name 'GetVolumeInformationW';
function GetVolumeInformationA(lpRootPathName : PAnsiChar; lpVolumeNameBuffer : PAnsiChar; nVolumeNameSize : DWORD;
	lpVolumeSerialNumber : PDWORD; lpMaximumComponentLength, lpFileSystemFlags : LPDWORD; lpFileSystemNameBuffer : PAnsiChar;
	nFileSystemNameSize : DWORD) : BOOL; stdcall; external kernel32 name 'GetVolumeInformationA';
{$ELSE}
function GetVolumeInformation(lpRootPathName : PChar; lpVolumeNameBuffer : PChar; nVolumeNameSize : DWORD; lpVolumeSerialNumber : PDWORD;
	lpMaximumComponentLength, lpFileSystemFlags : LPDWORD; lpFileSystemNameBuffer : PChar; nFileSystemNameSize : DWORD) : BOOL; stdcall;
	external kernel32 name 'GetVolumeInformationA';
{$ENDIF}
...
class function TFileSystem.HasFileExtLast(const _Filename : string; const _Ext : TStringArray) : Boolean;
var
	i : Integer;
begin
...
class function TFileSystem.CreateDir(const _DirectoryName : string; _ErrorHandling : TErrorHandlingEnum) : Boolean;
var
	LastError : Cardinal;
begin
...
class function TFileSystem.TryGetFileInfo(const _Filename : string; out _Info : TFileInfoRec) : Boolean;
var
	Sr : TSearchRec;
	Res : Integer;
begin
...
class function TFileSystem.TryGetFileSize(const _Filename : string; out _Size : Int64) : Boolean;
var
	Info : TFileInfoRec;
begin
...
class function TFileSystem.MoveFile(const _Source, _Dest : string; _ErrorHandling : TErrorHandlingEnum = ehRaiseException) : Boolean;
var
	LastError : Cardinal;
begin
...
const
	...
	{$IFEND}
	{$IF not declared(MOVEFILE_FAIL_IF_NOT_TRACKABLE)}
...
class function TFileSystem.MoveFileEx(const _Source, _Dest : string; _Flags : TMoveFileExFlagSet; _RaiseException : Boolean) : Boolean;
begin
	Result := Self.MoveFileEx(_Source, _Dest, _Flags, re2ehe(_RaiseException));
end;

class function TFileSystem.SetReadonly(const _Filename : string; _Value : Boolean; _RaiseException : Boolean) : Boolean;
begin
	Result := Self.SetReadonly(_Filename, _Value, re2ehe(_RaiseException));
end;

class function TFileSystem.SetReadonly(const _Filename : string; _Value : Boolean; _ErrorHandling : TErrorHandlingEnum) : Boolean;
var
	Attr : Integer;
	LastError : Cardinal;
begin
...
class function TFileSystem.CopyFile(const _Source, _Dest : string; _FailIfExists : Boolean; _RaiseException : Boolean = True;
	_ForceOverwrite : Boolean = False) : Boolean;
var
	LastError : Cardinal;
begin
...
class function TFileSystem.CopyFile(const _Source, _Dest : string; _IfExists : TCopyFileFlagIfExists; _ErrorHandling : TErrorHandlingEnum;
	_IfReadOnly : TCopyFileFlagOverwriteReadonly) : Boolean;
var
	LastError : Cardinal;
begin
...
begin
	...
	Result := Self.CopyFile(_Source, _Dest, IfExists, ErrorHandling, IfReadOnly);
end;

class function TFileSystem.AppendDateTime(const _Filename : string; _DateTime : TDateTime; _IncludeTime : Boolean) : string;
var
	BaseFilename : string;
	Ext : string;
begin
...
begin
	...
	while not Self.CopyFile(_Filename, Result, cfeFailIfExists, ehReturnFalse) do begin
		...
		if i = 1000 then begin
			...
			raise EBackupFailed.CreateFmt(_('Failed to create a backup of "%s". Tried 1000 different file names based on "%s".') + ' (%s)',
				[_Filename, BackupFilename.Filename, ErrorMessage]);
		end;
		Result := BackupFilename.DirectoryBS + BackupFilename.FilenameOnly + '_' + IntToStr(i) + BackupFilename.Extension;
	end;
end;
...
class function TFileSystem.CopyDir_Old(const _SrcDir, _DestDir : string; _CreateIntermediate, _RaiseException : Boolean) : Boolean;
var
...
class function TFileSystem.CopyDir(const _SrcDir, _DestDir : string; _CreateIntermediate : TCopyDirCreateIntermediate;
...
function ProgressCallback(_TotalFileSize, _TotalBytesTransferred, _StreamSize, _StreamBytesTransferred : LARGE_INTEGER;
	_StreamNumber, _CallbackReason : LongWord; _SourceFile, _DestinationFile : THandle; _Data : Pointer) : LongWord; far; stdcall;
var
...
begin
	try
		...
		case _CallbackReason of
			...
			Status.FCallbackReason := prStreamSwitch;
			else
...
class function TFileSystem.CopyFileWithProgress(const _Source, _Dest : string; _Progress : TCopyFileProgressEvt;
...
class function TFileSystem.CopyFileWithProgress(const _Source, _Dest : string; _Progress : TCopyFileProgressEvt;
	_IfExists : TCopyFileFlagIfExists; _ErrorHandling : TErrorHandlingEnum; _Restartable : TCopyFileWithProgressRestartable)
	: TCopyFileWithProgressResult;
var
...
begin
	...
	try
		...
		Res := Windows.CopyFileEx(PChar(_Source), PChar(_Dest), @ProgressCallback, Redir, @Redir.FCancelFlag, Flags);
...
class function TFileSystem.CopyMatchingFiles(const _Mask, _SrcDir, _DestDir : string; _Flags : TCopyFileFlagSet;
	_FilesSkipped : TStrings = nil) : Integer;
var
	i : Integer;
	Files : TStringList;
...
class function TFileSystem.MoveFileWithProgress(const _Source, _Dest : string; _Progress : TCopyFileProgressEvt;
	_Flags : TMoveFileWithProgressFlagSet = [mfwRaiseException]) : TCopyFileWithProgressResult;
var
	Redir : TProgressRedir;
	Flags : DWORD;
...
begin
	...
	try
		...
		if mfwFailIfNotTrackable in _Flags then
			Flags := Flags or MOVEFILE_FAIL_IF_NOT_TRACKABLE;
		Res := Windows.MoveFileWithProgress(PChar(_Source), PChar(_Dest), @ProgressCallback, Redir, Flags);
		if not Res then begin
			LastError := GetLastError;
...
class function TFileSystem.DeleteFile(const _Filename : string; _RaiseException : Boolean = True; _Force : Boolean = False) : Boolean;
var
	Attr : Integer;
	LastError : Cardinal;
begin
...
class function TFileSystem.DeleteMatchingFiles(const _Dir, _Mask : string; _RaiseException, _Force : Boolean) : Integer;
begin
	Result := DeleteMatchingFiles(_Dir, _Mask, [], _RaiseException, _Force);
end;

class function TFileSystem.DeleteMatchingFiles(const _Dir, _Mask : string; const _ExceptMask : string; _RaiseException, _Force : Boolean)
	: Integer;
begin
	Result := DeleteMatchingFiles(_Dir, _Mask, [_ExceptMask], _RaiseException, _Force);
end;

class function TFileSystem.DeleteMatchingFiles(const _Dir, _Mask : string; const _ExceptMasks : array of string;
...
class function TFileSystem.FindMatchingDirs(const _Mask : string; _sl : TStrings; _IncludePath : Boolean = False) : Integer;
var
	enum : TSimpleDirEnumerator;
begin
...
class function TFileSystem.FindMatchingFiles(const _Mask : string; _IncludePath : Boolean; _Sort : Boolean) : TStringArray;
var
	Cnt : Integer;
	sl : TStringList;
	i : Integer;
...
class function TFileSystem.FindMatchingFiles(const _Mask : string; _sl : TStrings; _IncludePath : Boolean = False; _Sort : Boolean = True)
	: Integer;
begin
...
class function TFileSystem.ForceDir(const _DirectoryPath : string; _ErrorHandling : TErrorHandlingEnum) : Boolean;
var
	LastError : Cardinal;
begin
	try
...
class function TFileSystem.ForceDirFor(const _FullFilename : string; _ErrorHandling : TErrorHandlingEnum) : Boolean;
var
	dir : string;
begin
...
class function TFileSystem.DelDirTree(const _DirName : string; _RaiseException, _Force : Boolean) : Boolean;
var
	Sr : TSearchRec;
	Filename : string;
begin
...
begin
	...
	Res := CreateFile(PChar(_Filename), GENERIC_READ or GENERIC_WRITE, FILE_SHARE_READ or FILE_SHARE_WRITE, nil, OPEN_EXISTING,
...
class function TFileSystem.IsValidFilename(const _s : string; out _ErrPos : Integer; _AllowDot : Boolean = True;
...
class function TFileSystem.IsValidFilename(const _s : string; _AllowDot : Boolean = True; _AllowPathChars : Boolean = False) : Boolean;
var
	ErrPos : Integer;
begin
...
class function TFileSystem.MakeValidFilename(const _s : string; _ReplaceChar : Char = '_'; _AllowPathChars : Boolean = True) : string;
var
	i : Integer;
...
procedure TDirectorySync.ProgressStatusCallback(_Status : TCopyProgressStatus; var _Continue : TCopyProgressResult);
begin
	try
			...
			if (EnumA.Sr.Attr and SysUtils.faDirectory) <> 0 then begin
				...
				if not SameText(ExtractFileExt(Filename), TMPEXT) then begin // do not sync .tmp files
					...
					if FileExists(Destination) then begin
						...
						doOnSyncingFile(FCurrentSource, FCurrentDest, EnumA.Sr.Size, 0);
						if cfwOK <> TFileSystem.MoveFileWithProgress(FCurrentSource, FCurrentDest, ProgressStatusCallback, [mfwFailIfExists])
						then
							SysUtils.Abort;
					end;
				end;
...
begin
	if not TryFindFile(_FilenameOnly, Result) then
		raise EFileNotFound.CreateFmt(_('Could not find file "%s" in search path "%s".'), [_FilenameOnly, Value]);
end;
