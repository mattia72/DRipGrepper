...
begin
	FRipGrepPath := TStringSetting.Create(RG_INI_KEY_RGPATH, '');
	FSearchPath := TStringSetting.Create(SEARCHPATH_KEY, '');
	FFileMasks := TStringSetting.Create(FILEMASKS_KEY, '');
	CreateSetting(FRipGrepPath);
	CreateSetting(FSearchPath);
	CreateSetting(FFileMasks);
		...
		if not rgExists then begin
			...
			if FileExists(scoopRgPath) then begin
				...
				if not sVsDir.IsEmpty then begin
					if sVsDir.EndsWith('\bin', True) then begin
						sVsDir := sVsDir.Remove(sVsDir.Length - '\bin'.Length);
					end;
					sVsDir := TFileUtils.ShortToLongPath(sVsDir);
					if TDirectory.Exists(sVsDir) then begin
