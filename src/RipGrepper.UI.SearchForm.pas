...
begin
	case rbExtensionOptions.ItemIndex of
		0 : begin
			...
			cmbSearchDir.Text := FSettings.ExtensionSettings.CurrentSearchSettings.ActiveFile;
		end;
		1 : begin
			cmbSearchDir.Enabled := False;
...
begin
	if not IOTAUTils.IsStandAlone then begin
		...
		TDebugUtils.DebugMessage('TRipGrepperSearchDialogForm.LoadExtensionSearchSettings ActiveFile=' + extSearchSettings.ActiveFile);
...
begin
	if IOTAUTils.IsStandAlone then begin
		...
		var
		shift := (rbExtensionOptions.Height + 2 * rbExtensionOptions.Margins.Bottom); //Margins.Top is 0
