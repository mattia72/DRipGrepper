...
- **extension** The IDE context (active project, project units, library path) was cached for the whole IDE session, so after switching the active project the *outside of project scope* indicators and the search path warning were still checked against the previous project. It is now invalidated by an IDE notifier (active project changed, project group opened/closed, package installed/uninstalled) and additionally re-checked against the IDE, so the result view is repainted with the paths of the new project.
